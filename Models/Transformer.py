...
class moving_token_inference(nn.Module):
...
class token_Transformer(nn.Module):
    def __init__(self, embed_dim=384, depth=14, num_heads=6, mlp_ratio=3.):
        ...
        self.moving_token = nn.Parameter(torch.zeros(1, 1, embed_dim))
        # self.contour_token = nn.Parameter(torch.zeros(1, 1, embed_dim))  # 后面改对称配准时这里需要考虑再加一个task token
        self.encoderlayer = token_TransformerEncoder(embed_dim=embed_dim, depth=depth, num_heads=num_heads, mlp_ratio=mlp_ratio)
        self.moving_token_pre = moving_token_inference(dim=embed_dim, num_heads=1)
        # self.contour_token_pre = contour_token_inference(dim=embed_dim, num_heads=1)

    def forward(self, moving_fea, fixed_fea):
        B, _, _ = moving_fea.shape
        fea_1_16 = torch.cat([moving_fea, fixed_fea], dim=2)
        fea_1_16 = self.mlp_s(self.norm(fea_1_16))   # [B, 14*14, 384]

        moving_tokens = self.moving_token.expand(B, -1, -1)
        fea_1_16 = torch.cat((moving_tokens, fea_1_16), dim=1)

        # contour_tokens = self.contour_token.expand(B, -1, -1)  # 后面改对称配准时再把这个任务相关的token加进去
        # fea_1_16 = torch.cat((fea_1_16, contour_tokens), dim=1)
        # fea_1_16 [B, 1 + 14*14 + 1, 384]

        fea_1_16 = self.encoderlayer(fea_1_16)
        # fea_1_16 [B, 1 + 14*14 + 1, 384]
        moving_tokens = fea_1_16[:, 0, :].unsqueeze(1)
        # contour_tokens = fea_1_16[:, -1, :].unsqueeze(1)

        moving_fea_1_16 = self.moving_token_pre(fea_1_16)
        # contour_fea_1_16 = self.contour_token_pre(fea_1_16)
        return moving_fea_1_16, fea_1_16, moving_tokens
